...
abstract class FirestoreDataSource<T extends Entity>
  ...
  FirestoreDataSource({required this.path, super.encryptor});
  ...
  /// ```
  @override
  Future<Response<T>> clear({DataFieldParams? params, Object? args}) async {
    return execute(() {
  ...
  @override
  Future<Response<int>> count({DataFieldParams? params, Object? args}) async {
    return execute(() {
  ...
  Future<Response<T>> getByIds(
    ...
    return execute(() {
      if (ids.length > _Limitations.whereIn) {
        ...
        return source(
          params,
        ).where(DataFieldPath.documentId, whereIn: ids).get().then((
          event,
        ) async {
  ...
  /// ```
  @override
  Stream<Response<int>> listenCount({DataFieldParams? params, Object? args}) {
    return executeStream(() {
      return Stream.fromFuture(
        source(params).count().get().then((e) {
