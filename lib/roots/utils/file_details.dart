...
class FileDetails {
  ...
  static String? getPrefix(String? path) {
    ...
    return FileFormats.prefix(getExtension(path));
  }
  ...
  String get filename => toFilename(extension: extension, path: path);

  /// Getter methods to determine the type of media data
  String get prefix {
    final ex = extension ?? getExtension(path);
    return FileFormats.prefix(ex) ?? "";
  }
  ...
  @override
  String toString() => "$FileDetails#$hashCode($json)";
}
...
enum FileFormats {
  ...
  static String? prefix(String? extension) {
    if (FileFormats.isAudioExt(extension)) {
      return "aud";
    } else if (FileFormats.isDocumentExt(extension)) {
      return "doc";
    } else if (FileFormats.isImageExt(extension)) {
      return "img";
    } else if (FileFormats.isVideoExt(extension)) {
      return "vid";
    } else {
      return null;
    }
