...
import '../../../../packages/imports.dart'
    show AndrossyExpandableText, AndrossyFlex, AndrossyImageGrid;
...
class ItemUserFeedMemory extends StatefulWidget {
  final PostModel item;
  final Function(BuildContext context, PostModel item)? onClick;

  const ItemUserFeedMemory({super.key, required this.item, this.onClick});

  @override
  State<ItemUserFeedMemory> createState() => _ItemUserFeedMemoryState();
}

class _ItemUserFeedMemoryState extends State<ItemUserFeedMemory> {
  String _title(UserModel user) {
    ...
    if (widget.item.isPublisher) {
      ...
      if (user.gender == Gender.male) {
        return "Update his memorable story at $date";
      } else {
        return "Update her memorable story at $date";
      }
    }
  }
            ...
            child: Padding(
              ...
              child: AspectRatio(
                ...
                child: DecoratedBox(
                  decoration: BoxDecoration(
                    color: dark.t01,
                    border: Border.all(color: Colors.white, width: dimen.dp(4)),
                  ),
                  child: ClipRRect(
